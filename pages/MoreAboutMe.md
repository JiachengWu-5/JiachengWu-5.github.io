---
layout: page
title: More About Me
description: Time flies
keywords: Jiacheng Wu, CV
comments: false
menu: More About Me
permalink: /more-about-me/
---
<!--
Note: permalink changed to '/more-about-me/' (slug form) to avoid URL encoding issues and to match nav hrefs without spaces.
If you prefer spaces, ensure `_config.yml` nav href uses the exact same string '/More About Me/' (including spaces) and the server handles encoded URLs.
-->

<br>

<left>
    <img src="/images/JiachengWu/JiachengWu.jpg" alt="picture not found" style="width:150px; height:auto;" />
    <br>
</left>

I am currently a PhD student in Computer Science at [HKBU](https://www.hkbu.edu.hk/). I obtained my MSc in Electronic Engineering from [HKUST](https://hkust.edu.hk/) in 2023, and received my BEng in Electronic Information Engineering from the School of Science and Engineering at [CUHKSZ](https://www.cuhk.edu.cn/en) in 2022.

<!-- My main research topics in the past five years have been computer vision and real applications scenery. I participated in several projects and realized the different computer vision model applications. I am also interested in the technology which combines NLP with CV, such as merging attention mechanisms in CV. The Transformer is a good example. I think multimodal learning tasks will be the next big trend for computer vision, like interdisciplinary integration. My further research intention is to develop a human-like computer vision system. From my perspective, integrating NLP and CV is similar to matching the computer vision model to the performance of the human eye, combined with the brain. I believe that in the future, computer vision models will compare the performance of the human eye. -->

## **Research Interests**

- Generative 3D Reconstruction
- Human Avatar Reconstruction
- Computer Graphic
- Computer Vision

## **Educations**

### 2023 – Present

- PhD Student in Computer Science
- The Hong Kong Baptist University [(HKBU)](https://www.hkbu.edu.hk/), Hong Kong, China
- Supervisor: [Prof. Jie CHEN](https://jchenhkg.github.io/)

### 2022 - 2023

- Master Degree
- Electronic Engineering, School of Electronic and Computer Engineering, The Hong Kong University of Science and Technology [(HKUST)](https://hkust.edu.hk/), Hong Kong, China
- CPA: 3.8, TGA: 3.8

### 2018 - 2022

- Bachelor of Engineering with Honour, Second Class Upper Division
- Electronic Information Engineering, School of Science and Engineering, The Chinese University of Hong Kong, Shenzhen [(CUHKSZ)](https://www.cuhk.edu.cn/en), Shenzhen, China.

## **Awards & Honors**

### Research Excellence

- **RPg Research Performance Award**, Dept. of Computer Science, HKBU, 2025–2026

### Teaching Excellence

- **Excellent Teaching Assistant Performance Award**, HKBU, 2025–2026 (Sem 1)
  - *Course:* COMP4025 Interactive Computer Graphics
- **Teaching Assistant Performance Award**, HKBU (4-time recipient, 2023–2026)
  - *COMP4025 Interactive Computer Graphics* (Sem 2, 2025–2026)
  - *COMP1015 Computing for Creatives I* (Sem 2, 2024–2025)
  - *COMP1015 Computing for Creatives I* & *COMP2037 Computing for Creatives II* (Sem 1, 2024–2025)
  - *COMP2036 AI and Data Analytics for Health and Social Innovation II* (Sem 2, 2023–2024)

## **Technical Skills**

- **Programming & Software**: Python, C++, Java, MATLAB, Web Development (HTML/CSS/JS, Three.js)
- **AI & 3D Computer Vision**: PyTorch, TensorFlow, OpenCV, CUDA, 3DGS/NeRF, Camera Calibration & Multi-View Capture
- **3D Engines & Creative Tech**: Unreal Engine, Unity, Blender, Adobe Premiere / Photoshop / Lightroom
- **Hardware & Sensing Systems**: Multi-Camera Rigs, Intel RealSense (RGB-D), Raspberry Pi, STM32, Arduino, FPGA
- **Productivity & Developer Tools**: Git, Linux, Bash, LaTeX, Markdown, ComfyUI / LLM Tooling

<!-- ## **Awards & Honors**

- Computer Association Scholarship
- The 3rd Prize in the World Drone Championship Campus Tour -->

## **Contact**

- **Email**: [csjcwu@comp.hkbu.edu.hk](mailto:csjcwu@comp.hkbu.edu.hk), [shanydoc@gmail.com](mailto:shanydoc@gmail.com)
- **Github**: [JiachengWu-5](https://github.com/JiachengWu-5)
- **WeChat**: wjc407171301


## **Beyond Work**

* **Birdwatching (观鸟)**: Passionate about spotting migratory and local birds along Hong Kong's trails. Nature is my best way to hit "system reset".
* **Social Responsibility (社会责任)**: Regular blood donor 🩸—a small, steady commitment to giving back.
* **Building & Tinkering (实用工程)**: Passionate about leveraging AI to bridge imagination and reality—from novel-to-video generation to web systems.

<!-- - My CV file is shown in [JiachengWu_CV.pdf](/assets/personal_file/Jiacheng_WU_CV.pdf). -->

<!-- ## Social -->

<!-- - 知乎@[沙漏](https://zhihu.com/people/zou-you-50)
- [Google Scholar](https://scholar.google.com.hk/citations?hl=zh-CN&user=etqLTR8AAAAJ)
- [Research Gate](https://www.researchgate.net/profile/Chaobin-Zou) -->

<!-- - Github: [JiachengWu-5](https://github.com/JiachengWu-5) -->
